		...
		if (inTrigger) {
			...
			RaycastHit[] hits;
				if (Input.GetKeyDown (KeyCode.E)) {
					...
					Debug.Log("pressed E");
					Debug.DrawRay(PlayerCam.position, PlayerCam.forward * 3, Color.yellow);
					Debug.Log(hits);	
					...
					if (hit.transform.tag == "ElevatorButtonOpen" && !isOpen) {
						Debug.Log("pressed ElevatorButtonOpen");
					...
					if (hit.transform.tag == "ElevatorNumericButton" && !Moving) {
						Debug.Log("pressed ElevatorNumericButton");
					...
					if (hit.transform.tag == "ElevatorGoButton" && !Moving) {
						Debug.Log("pressed ElevatorGoButton");
